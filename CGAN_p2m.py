...
def compress_images():
    ...
    """
    dataA = load_images("Trombi/", size=(150,200))
    dataB = load_images("Manga/", size=(225, 350))

    np.savez_compressed("t2m.npz", dataA, dataB)
    print("dataset saved as t2m.npz" )

def load_compressed_images():
    """
    Charge les images depuis la version compressée fabriquée plus haut
    """
    data = np.load('t2m.npz')
